...
- **You choose how you get there and how long you stay.** Click the transport emoji on an arrow to pick walk, train, bus, car, bike, boat or flight; click `+ stay` on a card to pick a stay. Both are saved into the note's hidden metadata for that line. Until you choose, transport is read from words on the line (巴士, 走, JR, 飛) or, failing that, guessed from the distance, and the guess is marked with a dashed border. Stays are never guessed: a station or an airport counts as zero, everything else waits for you.
...
- **Legs between stops are real.** Walking and driving legs are routed on OpenStreetMap (OSRM) and drawn along the road; hover a line for the leg's numbers. Transit legs use Google Routes when a key is set (line names included); otherwise they are estimated from the straight-line distance and marked `≈`. The timeline shows each leg between its two cards, the legend sums the day (stops, moving time, first to last time), and a leg that cannot fit between the two written times turns red with how many minutes late you would be.
- **A timetable you did not have to type.** Write a time only on the anchors (the bus, the reservation, the last entry) and set stays where they matter. Wayfarer walks the day forward: arrival = previous departure + leg, departure = arrival + stay. Where a stay is unknown the inference pauses until the next written time, so it never shows a time it made up. Inferred times show as `≈08:47`; a written anchor that cannot be reached turns red with the minutes you are short.
