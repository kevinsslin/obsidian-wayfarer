...
- **You choose how you get there.** Click the `?` on an arrow to pick walk, train, bus, car, bike, boat or flight, or type a transport emoji (🚌 🚶 🚃 ✈️) before the link. The choice is saved into the note's hidden `%%wf:{}%%` metadata for that line; the plugin never guesses and never edits your text.
