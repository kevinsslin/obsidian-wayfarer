...
- **A timeline under the map.** The active day's stops run top to bottom as cards (thumbnail, number, time, name, your remark), with an arrow between each pair carrying the leg: `🚶 34 min · 2.5 km`. Click a card and the map flies there, the popup opens, and the editor cursor lands on that line. Drag the divider to give the map or the timeline more room.
...
Interface language (follows Obsidian by default; English, 繁體中文, 日本語), paste conversion on/off, day heading level, per-day `tag:d1` tagging for Map View, route lines, auto-open, Google API key and language, tile URL and attribution (OpenStreetMap by default).
