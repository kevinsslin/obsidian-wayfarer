...
- **You choose how you get there.** Click the transport emoji on an arrow to pick walk, train, bus, car, bike, boat or flight; the choice is saved into the note's hidden metadata for that line. Until you choose, transport is read from words on the line (巴士, 走, JR, 飛) or, failing that, guessed from the distance, and the guess is marked with a dashed border.
...
- **Write it back.** The command *Write travel times into this note* appends ` · 🚶 34 分 · 2.5 km` to each stop's line (replacing an earlier one), so the plan reads on a phone without the map.
...
- **Write travel times into this note**
